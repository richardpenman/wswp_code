...
def regex_scraper(html):
    ...
    for field in FIELDS:
        results[field] = re \
            .search('<tr id="places_{}__row">.*?<td class="w2p_fw">(.*?)</td>'.format(field), html) \
            .groups()[0]
    return results
...
def lxml_scraper(html):
    ...
    for field in FIELDS:
        results[field] = tree \
            .cssselect('table > tr#places_{}__row > td.w2p_fw'.format(field))[0] \
            .text_content()
    return results
...
def main():
    ...
    for name, scraper in ('Regular expressions', regex_scraper), \
                         ('Beautiful Soup', beautiful_soup_scraper), \
                         ('Lxml', lxml_scraper):
        times[name] = []
        start = time.time()
